...
if __name__ == "__main__":
    ...
    ratings_with_gender_rdd = ratings_rdd.join(users_rdd)

    # ===== Bước 4: Đổi key thành (MovieID, Gender) =====
    # Kết quả: ((MovieID, Gender), (Rating, 1))
    movie_gender_rating_rdd = ratings_with_gender_rdd.map(
        lambda x: ((x[1][0][0], x[1][1]), (x[1][0][1], 1))
    )

    # ===== Bước 5: Tính tổng điểm và số lượt rating =====
    # Kết quả: ((MovieID, Gender), (sum_rating, count_rating))
    movie_gender_stats_rdd = movie_gender_rating_rdd.reduceByKey(
        lambda a, b: (a[0] + b[0], a[1] + b[1])
    )

    # ===== Bước 6: Tính điểm trung bình =====
    # Kết quả: ((MovieID, Gender), (avg_rating, count_rating))
    movie_gender_avg_rdd = movie_gender_stats_rdd.mapValues(
    ...
    # Đổi về dạng: (MovieID, (Gender, avg_rating, count_rating))
    temp_rdd = movie_gender_avg_rdd.map(
        lambda x: (x[0][0], (x[0][1], x[1][0], x[1][1]))
    )

    # ===== Bước 8: Join với movie title =====
    # Kết quả: (MovieID, ((Gender, avg_rating, count_rating), Title))
    result_rdd = temp_rdd.join(movies_rdd).map(
        lambda x: (x[1][1], x[1][0][0], x[1][0][1], x[1][0][2])
    )
    # (Title, Gender, avg_rating, count_rating)

    # Sắp xếp theo tên phim rồi theo giới tính
    result_rdd = result_rdd.sortBy(lambda x: (x[0], x[1]))

    print("=== ĐIỂM TRUNG BÌNH CỦA MỖI PHIM THEO GIỚI TÍNH ===")
    for title, gender, avg_rating, count_rating in result_rdd.collect():
        print(
            f"Title: {title} | Gender: {gender} | Avg rating: {avg_rating:.4f} | Count: {count_rating}"
        )
